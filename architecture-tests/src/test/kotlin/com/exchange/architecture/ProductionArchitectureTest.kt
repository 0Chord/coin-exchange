...
/** 실제 운영 출력에 규칙을 적용한다. P01·P02·P03은 각각 준비 조건을 확인하며 실행 순서에 의존하지 않는다. */
class ProductionArchitectureTest {
    ...
    fun `P02 전체 운영 코드와 Gradle 직접 선언에 ARCH-02를 적용한다`() {
        ...
        println("ARCH-06은 독립된 P03에서 평가합니다. 미평가 규칙: ARCH-03/04/05/07/08. 계산·DB·실행 순서 검증은 포함하지 않습니다.")
    ...
    fun `P03 실제 저장 발행 포트의 공개 계약에 ARCH-06을 적용한다`() {
        ...
        val scope = ProductionScopeImporter().load(ProductionScope.outputs(), expectations)
        ...
        println("한계: 미등록 포트의 의미, 임의 DTO 내부, 런타임 값·동작. 미평가 규칙: ARCH-03/04/05/07/08.")
    }
